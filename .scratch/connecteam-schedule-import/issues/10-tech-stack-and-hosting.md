...
## Update from issue 13 (2026-09-24)

[Issue 13](13-multi-tenant-hosted-operation-model.md) settled the *operating model* question that sits above this ticket's hosting-mechanics question: this project stays self-hosted-per-adopter only (ADR 0001 unchanged, no centrally-hosted mode holding real clients' keys — ruled out by Connecteam's actual API ToS, not just by preference). Within that model, it confirmed **two deployment targets should both be supported and shown to clients side by side**: today's local node+tunnel method, and a new Cloudflare Workers deployment, deployed to the adopter's own Cloudflare account — the client picks based on their own cost/technical-comfort tradeoff, the tool doesn't steer.

This **adds** a deployment target; it does **not** resolve this ticket's still-open question below. The local method's viability problem is untouched by it, and Cloudflare's own concrete mechanics (Workers vs. Durable Objects for the Relay's Chat-Link state, one Worker vs. split services, how a client actually deploys their own copy) still need their own design pass — likely a new ticket once this one's core "still fully open" question is addressed, since that question (stable public URL, no laptop required) is exactly what Cloudflare Workers would solve for free, and may reframe the sub-questions below rather than sit alongside them.

## Update from issue 14 (2026-09-25)

[Issue 14](14-cloudflare-deployment-mechanics.md) worked out Cloudflare's concrete mechanics: Relay ships first as a Cloudflare Worker + Durable Object (stable URL by construction, free-tier viable), Importer's Cloudflare version deferred (works on the Paid plan only — the free plan's 50-subrequest-per-invocation cap doesn't fit real Import Run volumes), and a **mixed** deployment shape is supported (Relay on Cloudflare, Importer local) alongside "both local" and "both Cloudflare."

**This narrows what's still actually open here**: the "stable public URL, no laptop required" problem below is now solved for any client who deploys the Relay to Cloudflare (mixed or full-Cloudflare shape) — a deployed Worker's URL persists by construction. What remains genuinely unresolved is scoped specifically to a client who wants to **stay fully local for both pieces**: the quick-tunnel problems (anonymous, rotating URL on every restart, doesn't survive reboot) are untouched, and a named/authenticated tunnel or reverse-proxy+DNS answer is still needed for that path if it's meant to be a real long-term option rather than only a demo/comparison configuration.
