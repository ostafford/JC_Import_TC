 ...
 * handled — first one returned wins — since nothing in this account or the
 * docs suggested that's a real scenario worth the extra complexity.
 */
...
async function writeRow(
  ...
  // No id to capture: per issue 08's live-corrected design, a break isn't linked to
  // its shift by reference — createBreakTimeActivity splits by time overlap instead.
 ...
 * breaks, never a real clock time (issue 01) — but Connecteam's write API
 * requires an actual start/end window per break (confirmed 2026-09-22,
 * correcting issue 08's original "write via durationMinutes" design, which
 * assumed the API accepted a bare duration). This derives a placement: every
 ...
 * not the employee's real break time — only the total duration is real, and
 * total worked/break time comes out correct either way; only where exactly
 * within the shift the split lands is a guess.
 */
function placeBreaksWithinShift(
  shiftStart: Date,
  shiftEnd: Date,
  breakMinutesInOrder: number[],
 ...
 * locked-day validation on Create Time Activities is a 400 with
 * `{"detail": ["User: X has locked days: [...]"]}` — corrects this project's
 * original guess of a 422 with an unspecified body shape.
 */
