import type { ConnecteamClient, ConversationId, PublisherId } from "@sch-import/shared";
...
export async function sendImportAbortedToChat(
  ...
  await client.postChatMessage({
    conversationId: config.conversationId,
    senderId: config.senderId,
    text: truncate(`⚠️ Import Run stopped: Connecteam rejected this Importer's credentials (${detail}). No shifts were imported. Check the Importer's API token.`, TEXT_CHAR_BUDGET),
  });
}
